...
func OpenSession(
	...
	termvisPath, err := resolveTermvisPath()
	if err != nil {
		return nil, OpenSessionOutput{Status: "error"}, err
	}
	...
	// termvis the instant OpenSession finished. The process must outlive
	// this call — its lifetime is managed via CloseSession/closeAllSessions.
	cmd := exec.Command(termvisPath, args...)
	...
	if err := cmd.Start(); err != nil {
		return nil, OpenSessionOutput{Status: "error"}, fmt.Errorf("failed to start termvis: %v", err)
	}
